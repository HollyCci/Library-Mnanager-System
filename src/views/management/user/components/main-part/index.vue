...
import {NSpace, NButton, NAvatar, NDropdown, NInput, NModal, NDescriptions, NDescriptionsItem, NSwitch} from 'naive-ui';
...
const columns: DataTableColumns<RowData> = [
	{
		...
		renderExpand: (rowData) => {
			return (
				<NDescriptions labelPlacement='left' bordered column={1} size='small' labelAlign='center'
											 labelStyle={'width:200px;text:center'}>
					<NDescriptionsItem label='备注'>
						{rowData.remark ? <t-tag theme="primary" variant="outline">{rowData.remark}</t-tag> :
							<t-tag theme="warning" variant="outline">无任何信息</t-tag>}
					</NDescriptionsItem>
					...
					<NDescriptionsItem label='最后登录IP'>
						{rowData.loginIp ? <t-tag theme="primary" variant="outline">{rowData.loginIp}</t-tag> :
							<t-tag theme="warning" variant="outline">从未登陆过</t-tag>}
					</NDescriptionsItem>
					<NDescriptionsItem label='最后登录时间'>
						{rowData.loginDate ? <t-tag theme="primary" variant="outline">{formatDate(rowData.loginDate)}</t-tag> :
							<t-tag theme="warning" variant="outline">从未登陆过</t-tag>}
					</NDescriptionsItem>
				</NDescriptions>
			)
	...
	{key: 'mobile', title: '手机号码', align: 'center', width: 120},
	{
		...
		align: 'center',
		render(row) {
			return <NSwitch value={row.status === 0} onChange={(value: boolean) => handleStatusChange(row, value)}/>
		}
	},
